...
VIDEO_FOLDER = "Simulation_Video"
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
VIDEO_PATH = os.path.join(VIDEO_FOLDER, f"firefighting_simulation_{timestamp}.mp4")
FRAME_LIST = []
...
Q_table = np.zeros((GRID_SIZE, GRID_SIZE, 3))
Q_table[:, :, 0] = 20
learning_rate = 0.2
discount_factor = 0.9
    ...
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            ...
            if 0 <= nx < GRID_SIZE and 0 <= ny < GRID_SIZE:
                if grid[nx, ny] == FIRE:
                    grid[nx, ny] = EMPTY
                    print(f"Fire extinguished at {nx}, {ny}")
...
def hose_extinguish_fire(x, y, state):
    ...
    if extinguished:
        ...
        if state["water_remaining"] == 0:
            state["needs_refill"] = True
        print(f"Hose extinguished fire at {x}, {y}, water remaining: {state['water_remaining']}")
        return True
    return False
...
def pass_water(giver_pos, receiver_pos):
    ...
    if giver_state["water_remaining"] > 1 and receiver_state["water_remaining"] < receiver_state["water_capacity"]:
        ...
        print(f"Water passed from {giver_pos} to {receiver_pos}, amounts: {giver_state['water_remaining']} -> {receiver_state['water_remaining']}")
        return True
    return False
...
def move_agents():
    ...
    for agent in agents:
        ...
        if action == 2 and state["water_remaining"] > 1:
...
def show_grid():
    ...
    if 'simulation_complete' in globals() and simulation_complete:
        ...
        text = font.render("All fires extinguished!!", True, (255, 255, 0))
...
def run_simulation():
    ...
    try:
        ...
        # Display completion message and capture additional frames
        completion_frames = FPS * 3
        ...
        # Save the video after the simulation ends
        save_video()
